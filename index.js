...
//msg.channel.send(r)
   if (r.indexOf(",")>-1){//有關鍵字
    ...
    t = ((c?c.length:0)+1)
    //msg.channel.send(p)
    //msg.channel.send("準備迴圈")
    while(i<t){
      //msg.channel.send("進入迴圈")
      if(r.indexOf(",")>-1){ //如果有,則放入陣列
        ...
        if(r.charAt(0)===","){ //如果重複打,就移除掉
          ...
          //msg.channel.send("r "+r)
          //msg.channel.send(p)
        }
      }else{ //最後一則
        //msg.channel.send("else")
        //msg.channel.send("else r "+r)
        l.push(r)
        //msg.channel.send(p)
      }
      //msg.channel.send("底部迴圈")
      i++;
    }
      //p=r.substring(0,r.indexOf(","))
     //msg.channel.send(p)
//msg.channel.send("脫離迴圈")
   }
 //i.push(p)
 //i.push(t)
 //i.push(q)
...
if(o === "?h"){
  ...
  if (o === "!hrandom") { //訊息為""則執行
  o=Math.floor(Math.random()*293905)+100000
  msg.reply(
     "https://nhentai.net/g/"+o+"/"
     ) //標註回覆網址
  }
  if (o==="!hr") {
    o=Math.floor(
  ...
  if (o === "嗨") { //訊息為""則執行
   msg.reply("嗨三小"); //標註回覆網址
  }
  if (o === "!on") { //訊息為""則執行
   msg.reply("運作中"); //標註回覆網址
  }
// if (o === "") { //訊息為""則執行
 //  msg.channel.send({embed:{
  //   color:0xffd700,
   //  title:"**指令列表**",
   //  description:"```實用性不高就是了```",
  //   fields:[{
   //   name:"!h <神的語言>",
   //   value:"```轉換成開車網址,並標注你這個小王八蛋 \n (ex: !h 000000 )```"
  //   },
  //   {
  //    name:"!hr , !hrandom",
  //    value:"```隨機本子,一樣標爆你```"
